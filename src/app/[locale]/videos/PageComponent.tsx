...
const PageComponent = ({
  ...
  return (
    <>
      ...
      <div className={"mb-8"}>
        <h2
          className={"text-blue-500 pt-4 text-4xl flex justify-center items-center font-semibold"}>{indexLanguageText.soraVideoExample}</h2>
      </div>
          ...
          {allVideoList.map((file) => (
            <div key={file.prompt}>
              ...
              <div className={"flex justify-center items-center"}>
                <p
                  className="pointer-events-none mt-2 block text-sm font-medium text-black w-[90%]">{indexLanguageText.prompt}: {file.prompt}</p>
              </div>
            </div>
          ))}
        </div>
